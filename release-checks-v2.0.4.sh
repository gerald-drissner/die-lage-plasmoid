...
grep -q '"Version": "2.0.4"' files/plasmoid/metadata.json
grep -q 'readonly property string appVersion: "2.0.4"' files/plasmoid/contents/ui/main.qml
grep -q '"version": "2.0.4"' files/bin/dielage-server.py
grep -q 'DieLage/2.0.4' files/bin/dielage-cache.py
grep -q '<release version="2.0.4" date=' files/plasmoid/metadata.appdata.xml
...
rsync -a ./ /tmp/die-lage-build-v${VERSION}/die-lage-v${VERSION}/   --exclude='.git'   --exclude='*.bak'   --exclude='*.swp'   --exclude='*.patch'   --exclude='__pycache__'   --exclude='*/__pycache__'   --exclude='*.pyc'   --exclude='die-lage-*.zip'   --exclude='die-lage-*.plasmoid'
...
rsync -a ./ /tmp/die-lage-latest-v${VERSION}/die-lage-latest/   --exclude='.git'   --exclude='*.bak'   --exclude='*.swp'   --exclude='*.patch'   --exclude='__pycache__'   --exclude='*/__pycache__'   --exclude='*.pyc'   --exclude='die-lage-*.zip'   --exclude='die-lage-*.plasmoid'
